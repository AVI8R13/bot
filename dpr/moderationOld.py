...
class Moderation(commands.Cog):
    ...
    @commands.command()
    async def sqltest(self, ctx, member: discord.Member, *, reason):
        userID = member.id
        modID = ctx.author.id
        serverID = ctx.message.guild.id
        caseMananger = ManageDatabase()
        caseMananger.createDb(serverID)
        ...
        banCaseNumber = caseMananger.getCases(serverID, caseInfo["caseType"])
        await ctx.send(banCaseNumber)
        caseMananger.updateCase(caseInfo, serverID)
        await ctx.send(f"{banCaseNumber} okay!")
